...
const CashFlowForecast = ({
  ...
  const ChartComponent = ({ chartData }: { chartData: CashFlowData[] }) => {
    const firstForecastIndex = chartData.findIndex((d) => d.isForecasted);

    const seriesData = chartData.map((d, idx) => {
      const isBoundaryPoint = firstForecastIndex > 0 && idx === firstForecastIndex - 1;

      return {
        ...d,
        actualSeries: d.actual,
        forecastSeries: d.isForecasted || isBoundaryPoint ? d.predicted : undefined,
      };
    });
